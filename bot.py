...
DB = Path(os.getenv("DB_PATH", "/app/data/fk.db"))
...
def init_db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS production (
        id TEXT PRIMARY KEY, user_id INTEGER, user_name TEXT, equipment TEXT,
        forming TEXT, product TEXT, start_time TEXT, end_time TEXT,
        quantity INTEGER, status TEXT, created_at TEXT)""")
    c.execute("""CREATE TABLE IF NOT EXISTS pauses (
        id TEXT PRIMARY KEY, user_id INTEGER, user_name TEXT, equipment TEXT,
        reason TEXT, start_time TEXT, end_time TEXT, created_at TEXT)""")
    c.commit(); c.close()
...
def verify_init_data(init_data: str):
    ...
    try:
        ...
        if not received or not auth_date:
            return None
        # Telegram Web Apps secret key: HMAC-SHA256(bot_token, key="WebAppData")
        secret = hmac.new(b"WebAppData", TOKEN.encode(), hashlib.sha256).digest()
        check = hmac.new(secret, "\n".join(f"{k}={pairs[k]}" for k in sorted(pairs)).encode(), hashlib.sha256).hexdigest()
        if not hmac.compare_digest(check, received):
            return None
        user = json.loads(pairs.get("user", "{}"))
        uid = int(user.get("id", 0))
        return {"id": uid, "name": user.get("first_name", "") + (" " + user.get("last_name", "") if user.get("last_name") else "")}
    except Exception:
        return None


def panel_keyboard(bot_username):
    # web_app buttons are not supported in group chats; direct Mini App link is.
    url = f"https://t.me/{bot_username}?startapp=production"
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="📱 Открыть ФК", url=url)]])
...
async def start(m: Message):
    if m.chat.type in ("group", "supergroup"):
        ...
        me = await m.bot.get_me()
        await m.answer("📊 Откройте ФК через кнопку рабочего чата.", reply_markup=panel_keyboard(me.username))


async def web_data(m: Message):
    # Kept only as a safety net for old keyboard launches.
    await m.answer("📱 Операции теперь выполняются в Mini App. Откройте ФК из рабочего чата.")
...
async def open_api(request: web.Request):
    ...
    c = db()
    rows = c.execute("SELECT id,equipment,forming,product,start_time FROM production WHERE user_id=? AND status='open' ORDER BY created_at DESC", (user["id"],)).fetchall()
    c.close()
...
async def save_api(request: web.Request):
    ...
    try:
        ...
        if event == "start":
            pid = str(uuid.uuid4())
            start_t = d.get("start") or now()
            c.execute("INSERT INTO production VALUES(?,?,?,?,?,?,?,?,?,?,?)", (pid, user["id"], user["name"], d.get("equipment", ""), d.get("group", ""), d.get("product", ""), start_t, None, None, "open", datetime.now(TZ).isoformat()))
            c.commit()
            out = {**d, "user_name": user["name"], "start_time": start_t}
            text = production_text(out)
        elif event == "finish":
            pid = d.get("open_id")
            q = int(str(d.get("quantity", "0")).replace(" ", ""))
            if not pid or q <= 0:
                raise ValueError("bad finish")
            row = c.execute("SELECT * FROM production WHERE id=? AND user_id=? AND status='open'", (pid, user["id"])).fetchone()
            if not row:
                return web.json_response({"error": "open_not_found"}, status=404)
            end_t = d.get("end") or now()
            c.execute("UPDATE production SET end_time=?, quantity=?, status='closed' WHERE id=?", (end_t, q, pid))
            c.commit()
            out = dict(row); out.update(user_name=user["name"], end_time=end_t, quantity=q)
            text = production_text(out, True)
        elif event == "pause":
            pid = str(uuid.uuid4()); start_t = d.get("start") or now(); end_t = d.get("end") or now()
            c.execute("INSERT INTO pauses VALUES(?,?,?,?,?,?,?,?)", (pid, user["id"], user["name"], d.get("equipment", ""), d.get("reason", ""), start_t, end_t, datetime.now(TZ).isoformat()))
            c.commit()
            text = pause_text({"user_name": user["name"], "equipment": d.get("equipment", ""), "reason": d.get("reason", ""), "start_time": start_t, "end_time": end_t})
        else:
            raise ValueError("unknown event")
        c.close()
        if CHAT:
            await request.app["bot"].send_message(int(CHAT), text, parse_mode="Markdown")
        return web.json_response({"ok": True})
    except Exception:
        logging.exception("save_api failed")
        return web.json_response({"error": "save_failed"}, status=500)


async def index(request: web.Request):
    return web.FileResponse("/app/webapp/index.html")


async def http_server(bot):
    app = web.Application(); app["bot"] = bot
    app.router.add_get("/", index)
    app.router.add_get("/api/open", open_api)
    app.router.add_post("/api/save", save_api)
    app.router.add_static("/static", "/app/webapp", show_index=False)
    runner = web.AppRunner(app); await runner.setup()
    site = web.TCPSite(runner, "0.0.0.0", PORT); await site.start()
    logging.info("Mini App server listening on %s", PORT)
    return runner
...
async def main():
    ...
    dp.message.register(panel, Command("setup_production"))
    dp.message.register(web_data, lambda m: m.web_app_data is not None)
    runner = await http_server(bot)
    try:
