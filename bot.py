...
BOT_TOKEN=os.getenv('BOT_TOKEN'); TIMEZONE=ZoneInfo(os.getenv('TIMEZONE','Europe/Moscow')); WORK_CHAT_ID_RAW=os.getenv('WORK_CHAT_ID','').strip(); WORK_CHAT_ID=int(WORK_CHAT_ID_RAW) if WORK_CHAT_ID_RAW else None
...
async def callbacks(c,s):
 ...
 if x.startswith('e:'):await s.clear();await s.update_data(event=x[2:]);await s.set_state(Form.equipment);await c.message.edit_text('🏭 **Выберите оборудование:**',reply_markup=kb([(v,f'q:{i}') for i,v in enumerate(EQUIPMENT)]+[('✏️ Другое','q:other')],2),parse_mode='Markdown')
  ...
  v=x[2:]
  if v=='manual':await c.message.edit_text('⌨️ Напишите время, например `08:30`.',parse_mode='Markdown')
  else:await s.update_data(start=now());await after_start(c.message,s)
 elif x.startswith('et:'):
  v=x[3:]
  if v=='manual':await c.message.edit_text('⌨️ Напишите время окончания, например `09:45`.',parse_mode='Markdown')
  ...
  d=await s.get_data();target=WORK_CHAT_ID or c.message.chat.id;await c.bot.send_message(target,result(d),parse_mode='Markdown');await s.clear();await c.message.edit_text('✅ **Готово! Итог опубликован.**',parse_mode='Markdown')
 await c.answer()
async def after_start(m,s):
 d=await s.get_data()
