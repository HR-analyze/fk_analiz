...
TOKEN=os.getenv("BOT_TOKEN","").strip(); TZ=ZoneInfo(os.getenv("TIMEZONE","Europe/Moscow")); CHAT=os.getenv("WORK_CHAT_ID","").strip(); PORT=int(os.getenv("PORT","8080")); DB=Path(os.getenv("DB_PATH","/app/data/fk.db")); DB.parent.mkdir(parents=True,exist_ok=True)
